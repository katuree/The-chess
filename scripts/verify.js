...
assert(authHtml.includes('profile-google-account') && authHtml.includes('username-google-account') && authJs.includes('renderAccount') && authJs.includes('getRedirectResult'), 'Auth flow should show the signed-in Google account automatically after login/redirect');
assert(authJs.includes('showSetupForm') && authJs.includes('Create an account with email') && authHtml.includes('id="auth-email"') && authHtml.includes('id="auth-panel" class="row"'), 'Auth page should visibly ask for email even before Firebase config or cached JS loads');
