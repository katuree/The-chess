...
assert(!landingHtml.includes('firebase-config.js') && !landingHtml.includes('auth-guard.js'), 'Landing page should not load the removed auth guard');
assert(!html.includes('firebase-config.js') && !html.includes('auth-guard.js'), '4-player page should not load the removed auth guard');
assert(!normalHtml.includes('firebase-config.js') && !normalHtml.includes('auth-guard.js'), 'Normal chess page should not load the removed auth guard');
assert(!normalRoomHtml.includes('firebase-config.js') && !normalRoomHtml.includes('auth-guard.js'), 'Normal room page should not load the removed auth guard');
assert(!normalQuickHtml.includes('firebase-config.js') && !normalQuickHtml.includes('auth-guard.js'), 'Normal quick match page should not load the removed auth guard');
assert(!fs.existsSync(path.join(root, 'auth.html')), 'auth.html should be removed');
assert(!fs.existsSync(path.join(root, 'scripts', 'firebase-config.js')), 'scripts/firebase-config.js should be removed');
assert(!fs.existsSync(path.join(root, 'scripts', 'auth.js')), 'scripts/auth.js should be removed');
assert(!fs.existsSync(path.join(root, 'scripts', 'auth-guard.js')), 'scripts/auth-guard.js should be removed');
